...
# There are three instances in US_Region and one instance in EU_Region. However, these instances are spread across three VPC networks (managementnet, mynetwork and privatenet), with no instance in the same zone and network as another. In the next section, you explore the effect this has on internal connectivity.
...
# Copied!
# Note: This should not work either as indicated by a 100% packet loss! You are unable to ping the internal IP address of managementnet-us-vm and privatenet-us-vm because they are in separate VPC networks from the source of the ping (mynet-us-vm), even though they are all in the same region US_Region.
...
# Note: For the below task consider region_1 = US_Region and region_2 = EU_Region`
