...
# student-04-dfe7502490b3@qwiklabs.net
# Copied!
...
# When you are connected, you are already authenticated, and the project is set to your Project_ID, qwiklabs-gcp-00-02869a6bdb3a. The output contains a line that declares the Project_ID for this session:

# Your Cloud Platform project in this session is set to qwiklabs-gcp-00-02869a6bdb3a
...
# ACTIVE: *
# ACCOUNT: student-04-dfe7502490b3@qwiklabs.net
...
# [core]
# project = qwiklabs-gcp-00-02869a6bdb3a
...
gcloud compute networks create managementnet --project=qwiklabs-gcp-00-02869a6bdb3a --subnet-mode=custom --mtu=1460 --bgp-routing-mode=regional

gcloud compute networks subnets create managementsubnet-us --project=qwiklabs-gcp-00-02869a6bdb3a --range=10.130.0.0/20 --stack-type=IPV4_ONLY --network=managementnet --region=us-east4
...
gcloud compute --project=qwiklabs-gcp-00-02869a6bdb3a firewall-rules create managementnet-allow-icmp-ssh-rdp --direction=INGRESS --priority=1000 --network=managementnet --action=ALLOW --rules=tcp:22,tcp:3389,icmp --source-ranges=0.0.0.0/0
...
gcloud compute instances create managementnet-us-vm --project=qwiklabs-gcp-00-02869a6bdb3a --zone=us-east4-c --machine-type=e2-micro --network-interface=network-tier=PREMIUM,stack-type=IPV4_ONLY,subnet=managementsubnet-us --metadata=enable-oslogin=true --maintenance-policy=MIGRATE --provisioning-model=STANDARD --service-account=353807774403-compute@developer.gserviceaccount.com --scopes=https://www.googleapis.com/auth/devstorage.read_only,https://www.googleapis.com/auth/logging.write,https://www.googleapis.com/auth/monitoring.write,https://www.googleapis.com/auth/servicecontrol,https://www.googleapis.com/auth/service.management.readonly,https://www.googleapis.com/auth/trace.append --create-disk=auto-delete=yes,boot=yes,device-name=managementnet-us-vm,image=projects/debian-cloud/global/images/debian-12-bookworm-v20240415,mode=rw,size=10,type=projects/qwiklabs-gcp-00-02869a6bdb3a/zones/us-east4-c/diskTypes/pd-balanced --no-shielded-secure-boot --shielded-vtpm --shielded-integrity-monitoring --labels=goog-ec-src=vm_add-gcloud --reservation-affinity=any
...
# Created [https://www.googleapis.com/compute/v1/projects/qwiklabs-gcp-04-972c7275ce91/zones/"us-east4-c"/instances/privatenet-us-vm].
...
# There are three instances in us-east4 and one instance in europe-west4. However, these instances are spread across three VPC networks (managementnet, mynetwork and privatenet), with no instance in the same zone and network as another. In the next section, you explore the effect this has on internal connectivity.
...
# ping -c 3 'Enter managementnet-us-vm internal IP here'
# Copied!
...
# ping -c 3 'Enter privatenet-us-vm internal IP here'
# Copied!
# Note: This should not work either as indicated by a 100% packet loss! You are unable to ping the internal IP address of managementnet-us-vm and privatenet-us-vm because they are in separate VPC networks from the source of the ping (mynet-us-vm), even though they are all in the same region us-east4.
...
# Note: For the below task consider region_1 = us-east4 and region_2 = europe-west4`
